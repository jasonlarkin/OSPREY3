...
# Diff prod-only lcov tracefiles (baseline vs fuzz) and report per-file deltas.
#
# Requires that you've run:
#   cmake --build build/cpp/kstar-coverage --target kstar_coverage_prod_compare_fuzz
#
# Usage:
#   ./scripts/kstar_lcov_diff_prod.sh [baseline_info] [fuzz_info]
#
# Default inputs:
#   build/cpp/kstar-coverage/coverage.prod.baseline.info
#   build/cpp/kstar-coverage/coverage.prod.fuzz.info
