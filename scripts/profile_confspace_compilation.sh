#!/bin/bash
# Profile ConfSpace compilation in isolation
# Measures: time per confspace, LEaP subprocess calls, parameterization overhead
...
# Defaults
CONFSPACE_FILE="${1:-}"
TEST_NAME="${2:-test_confspace}"

if [ -z "$CONFSPACE_FILE" ]; then
    echo "Usage: $0 <confspace_file> [test_name]"
    echo ""
    echo "Example:"
    echo "  $0 match5-processed-target-[MONTAGE].confspace target_profile"
    echo ""
    echo "To find confspace files:"
    echo "  find . -name '*.confspace' -type f"
    exit 1
fi

if [ ! -f "$CONFSPACE_FILE" ]; then
    echo "ERROR: ConfSpace file not found: $CONFSPACE_FILE"
    exit 1
fi

echo "=== ConfSpace Compilation Profiling ==="
echo "ConfSpace file: $CONFSPACE_FILE"
...
# Get absolute path
CONFSPACE_ABS=$(cd "$(dirname "$CONFSPACE_FILE")" && pwd)/$(basename "$CONFSPACE_FILE")
CONFSPACE_DIR=$(dirname "$CONFSPACE_ABS")
CONFSPACE_BASE=$(basename "$CONFSPACE_ABS" .confspace)

cd "$CONFSPACE_DIR"

# Create profiling script
cat > "$OUTPUT_DIR/$TEST_NAME/profile_compile.py" << 'PYEOF'
import sys
import os
import cProfile
import time
...
# Run profiling
echo "Running compilation profiling..."
cd "$REPO_ROOT"
CCKSTAR_DIR="$CCKSTAR_DIR" \
python3 "$OUTPUT_DIR/$TEST_NAME/profile_compile.py" \
    "$CONFSPACE_ABS" \
    "$OUTPUT_DIR/$TEST_NAME" \
    "$TEST_NAME" \
    2>&1 | tee "$OUTPUT_DIR/$TEST_NAME/output.txt"
...
echo "  - ${TEST_NAME}_profile.prof (cProfile binary)"
echo "  - ${TEST_NAME}_top50_cumulative.txt (top functions by cumulative time)"
echo "  - ${TEST_NAME}_top50_time.txt (top functions by self time)"
echo "  - ${TEST_NAME}_leap_calls.txt (LEaP subprocess call analysis)"
echo "  - ${TEST_NAME}_timing.txt (timing breakdown)"
echo "  - output.txt (full execution log)"
