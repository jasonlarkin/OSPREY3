...
# Reproduce a libFuzzer artifact/crash for the EnergyMatrixLoader target with consistent sanitizer settings.
#
# Usage:
#   ./scripts/kstar_fuzz_repro_energy_matrix_loader.sh <artifact_file> [fuzz_build_dir]
#
# Defaults:
...
if [[ "${ARTIFACT}" == *"<"* || "${ARTIFACT}" == *">"* ]]; then
  ...
  echo "use a real filename, e.g. build/cpp/kstar-fuzz/fuzz-artifacts/energy_matrix_loader/crash-123" >&2
  exit 2
fi
...
FUZZ_BIN="${FUZZ_BUILD_DIR}/fuzz_energy_matrix_loader"
if [[ ! -x "${FUZZ_BIN}" ]]; then
...
# Repro: run a single iteration over the provided artifact file.
